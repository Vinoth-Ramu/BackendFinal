...
const customerDetailsVar = require('../models/customerDetailsModel');
...
module.exports.createCustomer = async (req, res, next) => {
  ...
  var existUser = await customerDetailsVar.findOne({ mail: req.body.mail });
  ...
  var response = await customerDetails.save();
  res.send(response);
}
